...
class Feedbacks extends CI_Controller {
    ...
    public function index() {
        ...
        $arr['page'] = 'feedback';
        $arr['feedbackdata']=$val->result_array();
        $this->load->view('admin/vwManageFeedback',$arr);
    }

    public function reply($f_id) {
        
        $sql = "SELECT * FROM feedback WHERE f_id=$f_id;";
        $val = $this->db->query($sql);
        $arr['page'] = 'feedback';
        $arr['feedbackdata']=$val->result_array();
        $this->load->view('admin/vwReplyFeedback',$arr);
    }
